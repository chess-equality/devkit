...
func TestSetup_TwoRepos_TwoAgents(t *testing.T) {
	...
	makeRepoWithBare := func(name string) {
	...
	makeRepoWithBare("dumb-onion-hax")
	makeRepoWithBare("ouroboros-ide")
	...
	check := func(path string, wantBranch string) {
	...
	// dumb-onion-hax
	check(filepath.Join(devRoot, "dumb-onion-hax"), "agent1")
	check(filepath.Join(devRoot, paths.AgentWorktreesDir, "agent2", "dumb-onion-hax"), "agent2")
	// ouroboros-ide
	check(filepath.Join(devRoot, "ouroboros-ide"), "agent1")
	check(filepath.Join(devRoot, paths.AgentWorktreesDir, "agent2", "ouroboros-ide"), "agent2")
}
